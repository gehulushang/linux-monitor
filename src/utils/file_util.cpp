...
std::map<std::string, std::vector<std::string>> 
    FileUtil::RecursiveDirAll(const std::string dirname) {
...
std::vector<std::string> FileUtil::ListDir(const std::string dirname) {
    std::vector<std::string> results = list_dir(dirname, false);
    return results;
}

std::vector<std::string> FileUtil::ListDirAll(const std::string dirname) {
    std::vector<std::string> results = list_dir(dirname, true);
    return results;
}

std::string FileUtil::Join(const std::string prepath, const std::string subpath) {
    std::string path;
...
std::string FileUtil::Find(const std::string root, const std::string object) {
    std::string path{""};
    if (IsDir(root)) {
...
std::string FileUtil::GetContent(const std::string path) {
    std::string content{""};
    if (IsFile(path)) {
...
long FileUtil::GetSize(const std::string path) {
    long filesize(0);
...
std::int16_t FileUtil::replace(std::string& raw_str, const std::string oldvalue, 
...
bool FileUtil::startswith(const std::string raw_str, const std::string prefix) {
    bool res = true;
...
std::vector<std::string> FileUtil::list_dir(const std::string dirname, bool all) {
    std::vector<std::string> results;
    DIR * dirp = opendir(dirname.c_str());
    dirent * dp = NULL;
    std::size_t len = dirname.size();
